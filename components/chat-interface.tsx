...
import { Send, Loader2, Pill, Activity, Calendar, FileText, X } from 'lucide-react'
    ...
    <div className="space-y-3">
      ...
      {messages.length === 0 && (
        <div className="flex gap-2 flex-wrap">
          <button
            onClick={() => setSelectedCategory(selectedCategory === 'medications' ? null : 'medications')}
          ...
          <button
            onClick={() => setSelectedCategory(selectedCategory === 'history' ? null : 'history')}
            className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm flex items-center gap-1.5"
          >
          ...
          <button
            onClick={() => setSelectedCategory(selectedCategory === 'appointments' ? null : 'appointments')}
            className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm flex items-center gap-1.5"
          >
          ...
          <button
            onClick={() => setSelectedCategory(selectedCategory === 'vitals' ? null : 'vitals')}
            className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm flex items-center gap-1.5"
          >
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium flex items-center gap-2">
              {selectedCategory === 'medications' && <><Pill className="w-4 h-4" /> Medications</>}
              {selectedCategory === 'history' && <><FileText className="w-4 h-4" /> Medical History</>}
              {selectedCategory === 'appointments' && <><Calendar className="w-4 h-4" /> Appointments</>}
              {selectedCategory === 'vitals' && <><Activity className="w-4 h-4" /> Vitals</>}
            </h3>
            <button
