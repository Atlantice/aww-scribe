...
import { getPatient, getPatientAppointments } from '@/lib/firestore-helpers'
...
export async function POST(req: Request) {
  try {
    const { message, patientId, conversationHistory = [], model = 'Gemini 2.0 Flash' }: ChatRequest = await req.json()
    ...
    appointments.forEach(apt => {
      if (apt.soap?.medications) {
        apt.soap.medications.forEach(med => {
          // Add unique medications (simple deduplication by name)
          if (!activeMedications.find(m => m.name === med.name)) {
    ...
    const patientContext = buildPatientContext(patient, appointments, activeMedications)
...
- When discussing medications, always mention dosing and frequency
...
Answer the user's question based on this information.`
    ...
    const chatHistory = [
      ...
      {
        role: 'model' as const,
        parts: [{ text: `I'm ready to help with ${patient.name}'s care based on the medical records. What would you like to know?` }],
      },
    ]
...
`
  ...
  if (appointments.length > 0) {
    context += `\n\nRECENT MEDICAL HISTORY (Last ${Math.min(5, appointments.length)} visits):\n`
    appointments.slice(0, 5).forEach((apt, idx) => {
      const diagnoses = apt.soap?.diagnoses?.map(d => d.condition).join(', ') || 'None documented'
      const medsPresc = apt.soap?.medications?.map(m => m.name).join(', ') || 'None'
...
`
    ...
    if (latestVitals) {
      ...
      context += `Respiratory Rate: ${latestVitals.respiratoryRate || 'Not recorded'}\n`
...
function getModel(vertexAI: VertexAI, modelName: string) {
  ...
  switch (modelName) {
    ...
    case 'Sonnet 4.5':
      // For now, use Gemini 2.0 Flash (could integrate Claude API later)
      return vertexAI.getGenerativeModel({
        model: 'gemini-2.0-flash-exp',
        ...baseConfig,
      })
